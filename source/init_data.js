...
const exchange_limit = JSON.parse(fs.readFileSync('./data/source/exchange_limit.json', 'utf8'));
const timeframe_in_ms = JSON.parse(fs.readFileSync('./data/source/timeframe_in_ms.json', 'utf8'));
const pair_list = JSON.parse(fs.readFileSync('./data/source/pair_list.json', 'utf8'));
...
async function get_ohlcv(exchange, pair_name, timeframe, since_date, limit, timeframe_in_ms) {
    ...
    while (last_tf < now) {
        last_tf = last_tf + (limit) * timeframe_in_ms;
        if (last_tf < now) {
...
async function get_multi_ohlcv(exchange, pair_list, tf_list, start_date, exchange_limit_json, timeframe_in_ms_json) {
    for (const tf in tf_list) {
        for (const pair in pair_list) {
            await get_ohlcv(
                ...
                timeframe_in_ms_json[tf_list[tf]]
            );
        }
    }
}
